  ...
  return (
    <div style={{ minHeight: "100vh", paddingTop: "2rem", paddingBottom: "3rem" }}>
      <div className="container">
        <h4 className="fw-bold text-success mb-4">{t("profile.title")}</h4>
        ...
        <div className="row g-4">
              ...
              <div className="card-body text-center py-4">
                <div className="position-relative d-inline-block mb-3">
                  ...
                  <button
                    ...
                    style={{ bottom: 0, right: 0, width: "28px", height: "28px", padding: 0, fontSize: "0.75rem" }}
                ...
                <div className="text-start">
                  <small className="text-muted">{t("profile.memberSince")}</small>
                  <p className="mb-0 fw-bold">{new Date(user.createdAt).toLocaleDateString("it-IT")}</p>
                </div>
              </div>
            </div>

            <div className="d-flex flex-column gap-2">
              <button className="btn btn-success w-100" style={{ borderRadius: "10px" }}
                onClick={() => setShowEditForm(!showEditForm)}>
                {showEditForm ? t("profile.cancelEdit") : t("profile.editProfile")}
              </button>
              <button className="btn btn-outline-danger w-100" style={{ borderRadius: "10px" }}
          ...
          <div className="col-md-8">
            ...
            {showEditForm && (
              <div className="card shadow-sm border-0 mb-4" style={{ borderRadius: "16px", borderLeft: "4px solid #198754" }}>
                <div className="card-body">
                  <h6 className="fw-bold mb-3">{t("profile.editTitle")}</h6>
                  ...
                  <form onSubmit={handleUpdate}>
                    ...
                    <div className="mb-3">
                      ...
                      <input type="password" name="password" className="form-control"
                        placeholder={t("profile.passwordPlaceholder")} value={formData.password} onChange={handleChange} />
                    </div>
            ...
            {showDeleteConfirm && (
              <div className="card shadow-sm border-0 mb-4" style={{ borderRadius: "16px", borderLeft: "4px solid #dc3545" }}>
                <div className="card-body">
                  <h6 className="fw-bold text-danger mb-2">{t("profile.deleteTitle")}</h6>
                  <p className="text-muted small">{t("profile.deleteWarning")}</p>
            ...
            {!showEditForm && !showDeleteConfirm && (
              <div className="card shadow-sm border-0" style={{ borderRadius: "16px", background: "linear-gradient(135deg, #d4edda, #f0fff4)" }}>
                <div className="card-body text-center py-5">
                  <div style={{ fontSize: "3rem" }}>🌿</div>
                  <h5 className="fw-bold text-success mt-3">
                    {t("profile.welcome")}, {user.name.split(" ")[0]}!
                  </h5>
                  <p className="text-muted">{t("profile.welcomeText")}</p>
                </div>
              </div>
            )}
          </div>
